...
var order_location = {}
function next_() {
        TweenLite.to("#order_step_1", 0.5, {opacity:0, xPercent: -20, ease:Power2.easeInOut, onComplete: function() {
        ...
        create_button()
            ...
            if (!placemark) {
                placemark = new ymaps.Placemark(map.getCenter(), {
                    panelMaxMapArea: 0,
                    balloonContentBody: [
                        '<div class="map_output">',
                        '<div style="height: 120px; width: 100%">',
                        '<p id="balloon_address" style="color: black" ></p>',
                        '<input type="hidden" id="last_coordinates">',
                        '<button id="order_step_1_button" onclick="(function(e){ next_() })(event)" class="user_button_reverse next"> Далее <i class="fa fa-arrow-right" aria-hidden="true"></i></button>',
                        '</div>',
                        '</div>'
                    ].join(''),
                    }, {
            ...
            $(".address_item p").click(function() {
                const coordinates = $(this).closest(".address_item").find("input").val();
...
class order {
    ...
    change_state(state) {
        const condition = this.check_step();
        if (!condition) {
            return;
        }
        ...
        const old_step = "#order_step_" + (this.state + 1);
        const old_state = this.state;

        this.state = state;
        const new_step = "#order_step_" + (state + 1);
        ...
        const elem = $("#order_step_description"),
        elemPos = elem.offset().top - elem.outerHeight();
        
        TweenLite.to(window, 0.5, {scrollTo:elemPos, ease:Power2.easeInOut});

        TweenLite.to($(old_step), 0.5, {opacity:0, xPercent:-1*direction, ease:Power2.easeInOut, onComplete: function() {
        ...
        let progress = 1/3 * 100 * state + "%";

        TweenLite.to(".progress_line .fill", 0.5, {width:progress, onComplete: function() { 
...
if ($(window).width() >= 800) {
    ...
    let change_total_price = () => {

        let price_output =  delivery_value + price *  $("#number_value").val();
...
let order_time_last_change = () => {
    let time = $("#day").val() + " " + $("#order_time").val(); 
...
let change_order_price = () => {
    ...
    for (let product of products) {
        const name = $(product).find(".product_name").text(), price = parseFloat($(product).find(".product_price").text()),  value = $(product).find(".product_value").val();
...
$('#order_step_4_button').click(function(e) {
    ...
    if ($("#mark_favorite").hasClass("fa-star") ) {
        const favorite_address = $("#balloon_address").text();
        const favorite_coordinates = $("#last_coordinates").val();
        const house = " ";
        let object = {street:favorite_address,house:house,coordinates:favorite_coordinates};
...
$(".order_cart").click(function() {
    const container = $(this).parent().find(".product_input_container");
